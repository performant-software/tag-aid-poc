...
const MapView = (props) => {
    ...
    return (
        <React.Fragment>
            ...
            >
                ...
                <Typography variant="h6">{"Years 401 - 611"}</Typography>
    ...
    function parsePoints() {
        ...
        geoData.forEach((f) => {
            ...
            if (g && g.geometry && g.geometry.type === "Point") {
                const linkText =
                    g.properties.link.indexOf("pleiades") > -1
                        ? "Pleiades"
                        : g.properties.link.indexOf("geonames") > -1
                        ? "Geonames"
                        : "Other";
    ...
    function generateSectionLink(links) {
        ...
        links.forEach((link) => {
            ...
            if (lookup) {
                let titles = sections.find(
                    (s) => s.sectionId === lookup.sectionId
                );
                const sectionId = lookup.sectionId;
                const englishTitle =
                    titles && titles.englishTitle
                        ? titles.englishTitle.substring(0, 12)
                        : "";
